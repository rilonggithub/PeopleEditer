(function ($) {
    ...
    */
    ...
    var JQueryPeopleEditer_ENTER = 13;       /*  Enter  键*/
    ...
    **/
    function InitiatPeopleEditerIcon($source) {
        ...
        $($source).addClass('JQuery_Plug_PeopleEditer'); /*添加样式*/
    ...
    **/
    function InitiatPeopleEditerCheckName($source, dtd) {
        ...
        var _alias = '';
        $('div.JQuery_Plug_PeopleEditer_Alias:not([class*=AliasResolved],[class*=ResolveError],[class*=MultipleUnResolve])', $($source)).each(function () {
    ...
    /**
    *   函数名： OperationMultipleUnResolveAlias
    *   功能  ： 处理成功解析的但是有多个相同的Alias 的样式
    *
    *   参数
    ...
    *   
    **/
    function OperationMultipleUnResolveAlias(result, $source) {
        $('div.JQuery_Plug_PeopleEditer_Alias', $($source)).each(function () {
            var $alias = $(this);
            if ($($alias).text().toLowerCase() === (result.OriginalText[0] + ';') && ($($alias).find('ul').length === 0)) {
                var _html = "<ul class='MultipleUnResolveAlias_UL'>";
                for (var j = 0; j < result.OriginalText.length; j++) {
                    _html += "<li>" + result.DisplayName[j] + "</li>";
                }
                $(_html).appendTo($($alias));
                $($alias).addClass('JQuery_Plug_PeopleEditer_MultipleUnResolve');
                $($alias).attr('title', 'Multiple entrise matched.Please click to resolve.');
                $('ul.MultipleUnResolveAlias_UL', $($alias)).css({ 'top': $($alias).offset().top + 5 });
                $('ul.MultipleUnResolveAlias_UL li', $($alias)).each(function () {
                    var $li = $(this);
                    $($li).attr('title', '');
                    for (var j = 0; j < result.OriginalText.length; j++) {
                        if ($($li).text().indexOf(result.DisplayName[j]) > -1) {
                            $($li).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_OriginalText', result.OriginalText[j]);
                            $($li).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_Type', result.Type[j]);
                            $($li).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_DisplayName', result.DisplayName[j]);
                            $($li).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_AccountName', result.AccountName[j]);
                        }
                    }
                    $($li).click(function (event) {
                        var _type = $(this).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_Type');
                        var _accountName = $(this).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_AccountName');
                        var _displayName = $(this).data('JQuery_Plug_PeopleEditer_MultipleUnResolveAlias_LI_DisplayName');
                        RemovingDuplicateResolvedAlias(_displayName + ';', $source);

                        $($alias).removeClass('JQuery_Plug_PeopleEditer_ResolveError JQuery_Plug_PeopleEditer_MultipleUnResolve').removeClass('unresolve');
                        $($alias).text(_displayName + ';').addClass('JQuery_Plug_PeopleEditer_AliasResolved ' + _type);
                        $($alias).attr('title', _displayName);
    ...
    **/
    function AjaxOperationResolvedAlias(result, $source) {
        for (var i = 0; i < result.length; i++) {
            if (result[i] !== null) {
                if (result[i].DisplayName.length > 1) { OperationMultipleUnResolveAlias(result[i], $source); }
                else {
    ...
    **/
    function InitiatPeopleEditerClickEvent($source) {
        $($source).click(function (event) {
            $('ul.MultipleUnResolveAlias_UL').css('display', 'none');
    ...
    **/
    function InitiatAliasInputEvent($source) {
        ...
        .keydown(function (event) {
            ...
            else if (event.which === JQueryPeopleEditer_ENTER) {   /*  回车键*/
    ...
    function operationWhenPressUporDown($input, $source, direction) {
        ...
        if (direction === 'UP') {
            ...
            $('div.JQuery_Plug_PeopleEditer_Alias', $($source)).each(function () {
                ...
                if (((_aliasTop + _EXTRA_UP_TOP >= _UP_Top && _UP_Top + _EXTRA_UP_TOP >= _aliasTop) && $(this).offset().left <= _left && ($(this).offset().left + $(this).width() >= _left))
                    || ((_aliasTop + _EXTRA_UP_TOP >= _UP_Top && _UP_Top + _EXTRA_UP_TOP >= _aliasTop) && $(this).offset().left >= _left && $(this).offset().left <= _left + _EXTRA_LEFT_WIDTH)
                    || ((_aliasTop + _EXTRA_UP_TOP >= _UP_Top && _UP_Top + _EXTRA_UP_TOP >= _aliasTop) && $(this).offset().left + $(this).width() + _EXTRA_LEFT_WIDTH >= _left)
                    || ((_aliasTop + _EXTRA_UP_TOP >= _UP_Top && _UP_Top + _EXTRA_UP_TOP >= _aliasTop) && $(this).offset().left < _left && ($(this).next().offset().top > _aliasTop))
                ) {
    ...
    **/
    function positionInputByClick(_x, _y, $source, _html) {
        ...
        if ((_y >= _maxTop || _x > _maxRight) && (_isLoadInput === false)) {
            ...
            $($source).data('$InputPrevAliasDom', $($tempAliasDom));
        }
    }
